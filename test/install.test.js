...
test("doctor with isolated targets verifies only those targets", () => {
  ...
  try {
    ...
    installSkill({ packageRoot: ROOT, client: "codex", target: installed, packageVersion: "0.3.0" });

    const before = doctor({ packageRoot: ROOT, packageVersion: "0.3.0", targets: { codex: empty, claude: installed } });
