...
export function doctor({ packageRoot, packageVersion, targets = {} }) {
  const checks = [];
  const nodeMajor = Number.parseInt(process.versions.node.split(".")[0], 10);
  checks.push({
    name: "Node version",
    ok: nodeMajor >= MIN_NODE_MAJOR,
    message: `${process.versions.node} ${nodeMajor >= MIN_NODE_MAJOR ? "meets" : "does not meet"} >=${MIN_NODE_MAJOR}`
  });
  checks.push({
  ...
  for (const client of ["codex", "claude"]) {
    const target = targets[client] || defaultTargetForClient(client);
