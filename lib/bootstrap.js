...
const ATLASSIAN_URL_RE = /https?:\/\/(?:[^@\s)>\"]+@)?[A-Za-z0-9.-]+\.atlassian\.net\/[^\s)>"]+/g;
const NOTION_URL_RE = /https?:\/\/(?:[^@\s)>\"]+@)?(?:(?:www\.)?notion\.so|[A-Za-z0-9-]+\.notion\.site)\/[^\s)>"]+/g;
...
const PACKAGE_ROOT_FILES = new Set(["README.md", "INSTALL.md", "SKILL.md", "LICENSE", "SECURITY.md", "CHANGELOG.md", "MIGRATION.md"]);
const PACKAGE_DIR_PREFIXES = new Set([".github", "examples", "references", "docs", "evaluation", "case-studies", "schemas", "scripts", "test", "tests"]);
...
function isCredentialLikeQueryKey(key) {
  ...
  const tokenVariant = /^(?:access|refresh|id|auth|session|csrf|api|bearer|oauth2|x)?token$/.test(normalized);
  const accessKeyVariant = /^(?:aws)?accesskey(?:id)?$/.test(normalized)
    || /^(?:access|api|client|public|private|secret|x)?key(?:id)?$/.test(normalized) && normalized !== "key";
  return normalized.startsWith("auth")
    || ["jwt", "sig"].includes(normalized)
    || /^(?:api|x|client)?apikey$/.test(normalized)
    || accessKeyVariant
    ...
    || normalized.includes("clientassertion")
...
function sanitizeCandidateUrl(raw) {
  try {
    ...
    for (const key of Array.from(url.searchParams.keys())) {
      if (isCredentialLikeQueryKey(key)) url.searchParams.delete(key);
    }
    return url.toString();
  } catch {
    return "";
...
function sanitizeInputValue(raw) {
  ...
  return sanitizeCandidateUrl(value);
}

function sanitizedKnownSources(raw) {
  const values = Array.isArray(raw) ? raw : [];
  return values.map((item) => sanitizeInputValue(item)).filter(Boolean);
}
...
export function findCandidateSources(workspaceInput) {
  ...
  for (const path of walkFiles(workspace)) {
    ...
    for (const match of text.matchAll(NOTION_URL_RE)) {
      const value = sanitizeCandidateUrl(match[0]);
      if (value) candidates.push({ kind: "notion_url", value, path: pathRel });
    }
...
function collectContext(inputData, workspace, client) {
  return {
    anchor: sanitizeInputValue(inputData.anchor),
    ...
    known_sources: sanitizedKnownSources(inputData.known_sources),
