...
const IGNORED_SOURCE_PATHS = new Set(["scripts/eval_report.md"]);
...
const PACKAGE_ROOT_FILES = new Set(["README.md", "INSTALL.md", "SKILL.md", "CONTRIBUTING.md", "LICENSE", "SECURITY.md"]);
const PACKAGE_DIR_PREFIXES = new Set([".github", "examples", "references", "tests"]);
const PACKAGE_SPECIFIC_PATHS = new Set(["scripts/bootstrap_program_truth.py"]);
...
function isPackageContextPath(path, workspace) {
  ...
  return PACKAGE_ROOT_FILES.has(pathRel) || PACKAGE_SPECIFIC_PATHS.has(pathRel) || PACKAGE_DIR_PREFIXES.has(first);
}
...
function shouldScan(path, workspace, bootstrapContextRoots = []) {
  ...
  if (IGNORED_SOURCE_PATHS.has(pathRel)) return false;
