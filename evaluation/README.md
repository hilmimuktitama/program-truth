...
`contracts:verify` validates `examples/status-artifact.json` against the canonical `StatusArtifact` contract (byte-exact truth-tools schema copies), proves the documented `truth-tools review` command is present in README, SKILL, INSTALL, and the example report, and deep-compares the shipped schemas against the sibling truth-tools contracts. When the sibling repository is present, `npm test` additionally runs the real truth-tools review engine against the example and asserts `artifact_quality: pass` + `program_health: blocked` (`test/truth-tools-sibling.test.js`); without the sibling, that test skips cleanly.
